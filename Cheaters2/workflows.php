...
class Workflows {
	...
	*/
	public function toxml( $a=null, $format='array' ) {
		...
		if ( is_null( $a ) && !empty( $this->results ) ):
			$a = $this->results;
			//trigger_error("Here a is null and results not empty", E_USER_ERROR);
		elseif ( is_null( $a ) && empty( $this->results ) ):
			//trigger_error("Here a is null and results IS empty", E_USER_ERROR);
	...
	* @return none
	*/
	public function logit( $a )
	{
		$this->write($a, $this->logfile, FILE_APPEND );
	}
	/**
	* Description:
	* Accepts data and a string file name to store data to local file as cache
